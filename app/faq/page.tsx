...
// Single Accordion Item Component optimizing for low-end device transitions
const FAQAccordionItem = ({ 
  ...
  return (
    <div className={`bg-white rounded-2xl border ${isLowEnd ? '' : 'transition-all duration-300'} ${isOpen ? 'border-[#374bf9]/30 shadow-md' : 'border-slate-100 shadow-sm'}`}>
      ...
      {/* On low-end systems, skip rendering content/animation when closed to reduce DOM footprint and CPU layout loops */}
      {isLowEnd ? (
        isOpen ? (
...
function FAQContentPage() {
  ...
  // Static performance capability detection on mount
  useEffect(() => {
    ...
    if (isLowRam || isLowCores || isSlowConn) {
      setIsLowEnd(true);
      console.log("[FAQ] Low-end hardware or slow connection detected. Enabled optimizations.");
    }
  }, []);

  // Debounce search query update to eliminate typing stutter/lag on slow cores
  useEffect(() => {
  ...
  // Reset open accordion index when category or search changes
  useEffect(() => {
    setOpenIndex(null);
    ...
    <div className="min-h-screen bg-[#f6f5f8] flex flex-col font-sans relative">
      ...
      <main className="flex-1 w-full bg-[#f6f5f8]">
        ...
        <section className={`bg-gradient-to-r ${isLowEnd ? "from-[#1e3a8a] to-[#1e3a8a]" : "from-[#172554] to-[#1e3a8a]"} text-white py-14 px-6 lg:px-12 w-full relative overflow-hidden`}>
          {/* Visual overlay patterns - completely disabled on low-end systems */}
          {!isLowEnd && (
            <>
        ...
        <section className="max-w-[1600px] mx-auto px-6 lg:px-12 py-12 pb-24">
          <div className="flex flex-col lg:flex-row gap-12 items-start">
            ...
            {/* Left Column: Mobile Search / Sticky Additional Help Cards */}
            <aside className="w-full lg:w-1/4 lg:sticky lg:top-24 flex flex-col gap-6">
              ...
              <div className="bg-white rounded-3xl border border-slate-200/60 p-6 shadow-sm flex flex-col gap-5 w-full">
                ...
                <div className="bg-slate-50 rounded-2xl p-4 border border-slate-150/50">
                  <h4 className="text-xs font-extrabold text-slate-850 flex items-center gap-2 mb-2">
            ...
            {/* Right Column: Accordion Items Grid */}
